...
public class GraphPetriTransition extends GraphTransition implements Serializable {
    ...
    public void drawGraphElement(Graphics2D g2) {
    	if(transition.getPriority() < 10 && transition.getPriority()>=0)
            ...
            if(transition.getPriority() >= 10) 
                setWidth(15);
    	super.drawGraphElement(g2);
        int font = 10;
        g2.drawString(transition.getName(), 
                (float) this.getGraphElement().getCenterX() - transition.getName().length() * font / 2, 
                (float) this.getGraphElement().getCenterY() - GraphPetriTransition.getHEIGHT()/ 2-GraphPetriTransition.getHEIGHT()/5);
